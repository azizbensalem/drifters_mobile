...
import { removeStatistique, fetchStatistiques } from "../../services/statistique.service";
...
export default function StatisticScreen({ navigation }) {
    ...
    return (
      <HStack flex="1" pl="2">
        <Pressable
          ...
          onPress={() =>
            navigation.navigate("Modifier Statistique", { data: data.item })
          }
          _pressed={{
            opacity: 0.5,
          }}
  ...
  const renderItem = ({ item, index }) => (
    <Box>
      <Pressable
        onPress={() => navigation.navigate("Afficher statistque", { data: item })}
        _dark={{
          bg: "coolGray.800",
        }}
        _light={{
